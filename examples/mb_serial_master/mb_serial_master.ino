...
#define TEST_ARR_REG_SZ (5) // Size of test_regs array used in the example
...
static void master_operation_func(void *arg)
{
    ...
    for(uint16_t retry = 0; retry <= MASTER_MAX_RETRY && (!alarm_state); retry++) {
        ...
        for (uint16_t cid = 0; (err != ESP_ERR_NOT_FOUND) && cid < MASTER_MAX_CIDS; cid++)
        {
            ...
            ESP_LOGI(TAG, "Get info for characteristic #%d, err = 0x%x", cid, err);
...
static esp_err_t master_init(void)
{
    ...
    vTaskDelay(5);
    ESP_LOGI(TAG, "Modbus master stack initialized, adding %d parameters...", num_device_parameters);
